...
int main(void) {
    ...
    status = mizu_session_attach_tokens(NULL, &decoded_token, 1, MIZU_ATTACH_FLAG_NONE);
    if (!expect_status("null session attach tokens", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    status = mizu_session_attach_modal_input(NULL, &modal_input);
    if (!expect_status("null session attach modal input", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    ...
    status = mizu_session_prefill(NULL, &report_buffer);
    if (!expect_status("null session prefill", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    if (!expect_true("null session prefill should preserve report buffer inputs",
                     report_buffer.struct_size == sizeof(report_buffer) &&
                     report_buffer.reports == report_storage &&
                     report_buffer.report_capacity == 2)) return 1;
    if (!expect_true("null session prefill should clear report count", report_buffer.report_count == 0)) return 1;
    if (!expect_true("null session prefill should clear report payloads",
                     report_storage[0].struct_size == sizeof(report_storage[0]) &&
    ...
    report_storage[0].cache_flags = UINT64_C(9);
    report_storage[0].elapsed_us = UINT64_C(9);
    status = mizu_session_decode_step(NULL, &decode_options, &decode_result, &report_buffer);
    if (!expect_status("null session decode", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    if (!expect_true("null session decode should preserve result inputs",
                     decode_result.struct_size == sizeof(decode_result) &&
                     decode_result.token_buffer == &decoded_token &&
                     decode_result.token_capacity == 1)) return 1;
    if (!expect_true("null session decode should clear result outputs",
                     decode_result.token_count == 0 &&
                     decode_result.stop_reason == MIZU_STOP_REASON_NONE &&
                     decode_result.result_flags == 0)) return 1;
    if (!expect_true("null session decode should preserve report buffer inputs",
                     report_buffer.struct_size == sizeof(report_buffer) &&
                     report_buffer.reports == report_storage &&
                     report_buffer.report_capacity == 1)) return 1;
    if (!expect_true("null session decode should clear report count", report_buffer.report_count == 0)) return 1;
    if (!expect_true("null session decode should clear report payload",
                     report_storage[0].struct_size == sizeof(report_storage[0]) &&
                     report_storage[0].stage_kind == 0 &&
                     report_storage[0].backend_family == 0 &&
                     report_storage[0].execution_route == 0 &&
    ...
    status = mizu_session_read_output(NULL, &output_buffer);
    if (!expect_status("null session read output", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    if (!expect_true("null session read output should preserve inputs",
                     output_buffer.struct_size == sizeof(output_buffer) &&
                     output_buffer.output_kind == MIZU_OUTPUT_KIND_TOKEN_IDS &&
                     output_buffer.data == &output_token &&
                     output_buffer.byte_capacity == sizeof(output_token))) return 1;
    if (!expect_true("null session read output should clear outputs",
    ...
    report_storage[0].cache_flags = UINT64_C(9);
    report_storage[0].elapsed_us = UINT64_C(9);
    status = mizu_session_park(NULL, &report_buffer);
    if (!expect_status("null session park", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    if (!expect_true("null session park should preserve report buffer inputs",
                     report_buffer.struct_size == sizeof(report_buffer) &&
                     report_buffer.reports == report_storage &&
                     report_buffer.report_capacity == 1)) return 1;
    if (!expect_true("null session park should clear report count", report_buffer.report_count == 0)) return 1;
    if (!expect_true("null session park should clear report payload",
                     report_storage[0].struct_size == sizeof(report_storage[0]) &&
                     report_storage[0].stage_kind == 0 &&
                     report_storage[0].backend_family == 0 &&
                     report_storage[0].execution_route == 0 &&
    ...
    report_storage[0].cache_flags = UINT64_C(9);
    report_storage[0].elapsed_us = UINT64_C(9);
    status = mizu_session_resume(NULL, &report_buffer);
    if (!expect_status("null session resume", status, MIZU_STATUS_INVALID_ARGUMENT)) return 1;
    if (!expect_true("null session resume should preserve report buffer inputs",
                     report_buffer.struct_size == sizeof(report_buffer) &&
                     report_buffer.reports == report_storage &&
                     report_buffer.report_capacity == 1)) return 1;
    if (!expect_true("null session resume should clear report count", report_buffer.report_count == 0)) return 1;
    if (!expect_true("null session resume should clear report payload",
                     report_storage[0].struct_size == sizeof(report_storage[0]) &&
                     report_storage[0].stage_kind == 0 &&
                     report_storage[0].backend_family == 0 &&
                     report_storage[0].execution_route == 0 &&
